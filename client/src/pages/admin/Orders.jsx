          ...
          {['all', ...STATUSES].map(s => (
            <button
              ...
              className={`flex-shrink-0 px-4 py-1.5 text-[10px] tracking-[2px] uppercase border transition-colors ${
                filter === s ? 'border-[#C9A84C] text-[#C9A84C]' : 'border-[#007605] text-[#555] hover:text-white hover:border-[#555]'
              }`}
            >
              {s}
            </button>
            ...
            {filtered.map(o => (
              <button
                ...
                className={`w-full text-left p-4 border transition-colors ${
                  selected?.id === o.id ? 'border-[#C9A84C] bg-[#003e02]' : 'border-[#005b04] bg-[#003e02] hover:border-[#007605]'
                }`}
              >
                ...
                <div className="mb-5">
                  ...
                  <div className="space-y-2">
                    {selected.items?.map((item, i) => (
                      <div key={i} className="flex justify-between text-sm py-2 border-b border-[#005403]">
                    ...
                    {['unpaid', 'paid', 'refunded'].map(p => (
                      <button
                        ...
                        className={`text-[10px] tracking-[2px] uppercase px-3 py-1.5 border transition-colors disabled:opacity-30 disabled:cursor-default ${
                          selected.payment_status === p ? PAYMENT_COLORS[p] : 'border-[#007605] text-[#555] hover:border-[#555] hover:text-white'
                        }`}
                      >
                        {p}
                      </button>
                      ...
                      <button
                        ...
                        className={`text-[10px] tracking-[2px] uppercase px-3 py-1.5 border transition-colors disabled:opacity-30 disabled:cursor-default ${
                          selected.status === s
                            ...
                            : 'border-[#007605] text-[#555] hover:border-[#555] hover:text-white'
                        }`}
                      >
                        {s}
                      </button>
